...
- **暂停不会让它消失** —— 显隐只看滤镜是否还在
- **参数实时生效** —— 拖动滑块立刻作用于下一帧 / 暂停时作用于松手后重新交付的那一帧，同时写回本机的 `dlssnr_dshow.ini`（不进仓库）
...
| 文件 | 角色 |
| --- | --- |
| `app\dlssnr_dshow.ini.template` | **模板**，随仓库分发，永远保持文档里写的默认值。想手动改配置就把它复制成 `dlssnr_dshow.ini` |
| `app\dlssnr_dshow.ini` | **运行时配置**：面板每次落定改动都会写回这里；这个文件**不进仓库**（已在 `.gitignore` 中）。不存在时滤镜用内置默认值 |

这样"文档默认值"和"你本机调好的数值"不会互相污染。
...
面板里改的会实时生效；拖动结束或切换选项时写回本机的 `dlssnr_dshow.ini`（模板是 `dlssnr_dshow.ini.template`，仓库里那份永远保持默认值），下次启动读取。
